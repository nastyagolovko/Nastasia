...
(https://github.com/nastyagolovko/Nastasia#линейные-классиикаторы)
...
Требуется найти ветор параметра w, при котором остточно мнимма аппроксимирваного эмпирического риска.
Пррименяем мето градиентноо спуска. В этом методе выбирается некоторое начальное приближение для w, затем запускается интерационный процесс, на каждом шаге которго вектор w изменяется в направлении наиболее быстрого убывания функционала Q. 
